...
//   // URL 파라미터에서 hobby_place와 hobby_id를 가져옴
//   const urlParams = new URLSearchParams(window.location.search);
//   const hobbyPlace = urlParams.get('hobby_place');
//   const hobbyId = urlParams.get('hobby_id');
  
//   // hobbyPlace와 hobbyId가 존재하면 검색 결과를 표시하고, 지도의 중심을 사용자의 위치로 설정
...
//       // 현재 키워드가 존재하면 해당 위치를 중심으로 장소를 검색
//       if (currentKeyword) { searchService.searchByLocation(locPosition, 5000, currentKeyword); }
//     }, function(error) { alert("사용자의 위치를 가져올 수 없습니다. 위치 접근을 허용해 주세요."); }); // 위치 접근 실패 시 알림
//   } else { alert("Geolocation을 지원하지 않는 브라우저입니다."); } // 브라우저가 Geolocation을 지원하지 않을 때 알림
// }
